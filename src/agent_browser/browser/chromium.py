...
def get_binary_path(platform_tag: Optional[str] = None) -> Path:
    ...
    If CLOAKBROWSER_BINARY_PATH is set:
      - If it points to a file, use it directly
      - If it points to a directory, search for chrome/chromium inside it
    """
    custom = os.environ.get("CLOAKBROWSER_BINARY_PATH")
    if custom:
        path = Path(custom).expanduser()
        # If pointing to a file, use directly
        if path.is_file():
            return path
        # If pointing to a directory, search for the binary
        if path.is_dir():
            tag = platform_tag or get_platform_tag()
            if tag.startswith("darwin"):
                candidates = ["Chromium", "chrome", "chromium"]
            elif tag.startswith("windows"):
                candidates = ["chrome.exe", "chromium.exe"]
            else:
                candidates = ["chrome", "chromium"]
            # Search in directory and subdirectories (max 2 levels)
            for name in candidates:
                # Direct child
                direct = path / name
                if direct.is_file():
                    return direct
                # One level deep (e.g. chromium-146.xxx/chrome)
                for subdir in path.iterdir():
                    if subdir.is_dir():
                        ...
                        # macOS .app bundle
                        if tag.startswith("darwin"):
                            app_binary = subdir / "Chromium.app" / "Contents" / "MacOS" / "Chromium"
                            if app_binary.is_file():
                                return app_binary
            # Not found in directory - return expected default path
            logger.warning(
                "CLOAKBROWSER_BINARY_PATH=%s is a directory but no chrome binary found inside. "
                "Set it to the chrome executable path, e.g. %s/chrome",
                custom, custom,
            )
...
def download_binary(
    ...
    if custom:
        ...
        if path.is_file():
            ...
            return path
        if path.is_dir():
            # Directory exists - search for binary inside
            resolved = get_binary_path(tag)
            if resolved.is_file():
                logger.info("Using custom binary: %s", resolved)
                return resolved
            # Directory exists but no binary - proceed with download into default location
            logger.warning(
                "CLOAKBROWSER_BINARY_PATH=%s is a directory with no chrome binary. "
                "Downloading to default location instead.",
                custom,
            )
        elif not path.exists():
            logger.warning(
                "CLOAKBROWSER_BINARY_PATH=%s does not exist. "
                "Downloading to default location.",
                custom,
            )
